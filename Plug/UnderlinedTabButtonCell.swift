...
class UnderlinedTabButtonCell: NSButtonCell {
	...
	var fixedTextFrame = NSRect(x: 5, y: 40, width: 50, height: 15)
	...
	override func drawBezel(withFrame frame: NSRect, in controlView: NSView) {
		if state == .on {
			let highlightRect = NSRect(x: 0, y: frame.size.height - hightlightWidth, width: frame.size.width, height: hightlightWidth)
			highlightColor.set()
			highlightRect.fill()
		}
	}

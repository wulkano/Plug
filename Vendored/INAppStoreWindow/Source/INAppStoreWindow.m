...
NS_INLINE void INApplyClippingPathInCurrentContext(CGPathRef path) {
	INApplyClippingPath(path, [[NSGraphicsContext currentContext] CGContext]);
}
...
- (void)drawWindowBackgroundLayersInRect:(NSRect)drawingRect forEdge:(NSRectEdge)drawingEdge showsSeparator:(BOOL)showsSeparator clippingPath:(CGPathRef)clippingPath
{
	...
	if ((window.styleMask & NSWindowStyleMaskTexturedBackground) == NSWindowStyleMaskTexturedBackground) {
...
- (void)drawRect:(NSRect)dirtyRect
{
	...
	if (window.showsTitle && ((window.styleMask & NSWindowStyleMaskFullScreen) == 0 || window.showsTitleInFullscreen)) {
...
- (void)drawNativeWindowTitleInRect:(NSRect)titleTextRect
{
	...
	HIThemeDrawTextBox((__bridge CFTypeRef)(window.title), &titleTextRect, &titleTextInfo, [[NSGraphicsContext currentContext] CGContext], self.isFlipped ? kHIThemeOrientationNormal : kHIThemeOrientationInverted);
}
...
- (void)getTitleFrame:(out NSRect *)frame textAttributes:(out NSDictionary **)attributes forWindow:(in INAppStoreWindow *)window
{
	...
	if (isUsingCustomTitleDrawing)
	{
		...
		NSFont *titleFont = window.titleFont ?: [NSFont titleBarFontOfSize:[NSFont systemFontSizeForControlSize:NSControlSizeRegular]];
...
- (void)mouseDragged:(NSEvent *)theEvent
{
	...
	if (window.isMovableByWindowBackground || (window.styleMask & NSWindowStyleMaskFullScreen) == NSWindowStyleMaskFullScreen) {
...
- (void)setTitleBarHeight:(CGFloat)newTitleBarHeight adjustWindowFrame:(BOOL)adjustWindowFrame
{
	if (_titleBarHeight != newTitleBarHeight) {
		...
		if ((self.styleMask & NSWindowStyleMaskTexturedBackground) == NSWindowStyleMaskTexturedBackground) {
...
- (void)setStyleMask:(NSWindowStyleMask)styleMask
{
	...
	if (((self.styleMask & NSWindowStyleMaskTexturedBackground) == NSWindowStyleMaskTexturedBackground) &&
		((styleMask & NSWindowStyleMaskTexturedBackground) != NSWindowStyleMaskTexturedBackground)) {
...
- (NSButton *)_fullScreenButtonToLayout
{
	return [self _windowButtonToLayout:NSWindowZoomButton orUserProvided:self.fullScreenButton];
}
